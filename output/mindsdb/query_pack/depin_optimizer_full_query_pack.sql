...
-- Generated for mode: full
-- Expected table: depin_optimizer_full_dataset
...
FROM depin_optimizer_full_dataset
WHERE mode = 'full' AND gate_pass = false
ORDER BY baseline_min_solvency ASC, protocol_name ASC
LIMIT 10;

-- 2) Sensitivity hotspots (all protocols)
WITH sensitivity AS (
  SELECT chain, protocol_id, sensitivity_1_parameter AS factor, sensitivity_1_delta AS delta FROM depin_optimizer_full_dataset WHERE mode = 'full' AND sensitivity_1_parameter <> ''
  UNION ALL
  SELECT chain, protocol_id, sensitivity_2_parameter AS factor, sensitivity_2_delta AS delta FROM depin_optimizer_full_dataset WHERE mode = 'full' AND sensitivity_2_parameter <> ''
  UNION ALL
  SELECT chain, protocol_id, sensitivity_3_parameter AS factor, sensitivity_3_delta AS delta FROM depin_optimizer_full_dataset WHERE mode = 'full' AND sensitivity_3_parameter <> ''
)
...
WITH failing AS (
  SELECT * FROM depin_optimizer_full_dataset WHERE mode = 'full' AND gate_pass = false
), sensitivity AS (
